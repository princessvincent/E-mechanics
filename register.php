...
if(isset($_POST["sub"])){
    if(isset($_POST["fullname"]) && isset($_POST["username"]) && isset($_POST["email"]) && isset($_POST["phone"]) && isset($_POST["shop"]) && isset($_POST["role"]) && isset($_POST["state"])  && isset($_POST["cit"])  && isset($_POST["password"])){
...
$ret = mysqli_query($conn,$sq2);
        ...
        if(mysqli_num_rows($rest) ==1 ){
            ...
            $insert = "INSERT INTO register (fullname,username,email,phone,role_id,shop_address,state_id,city,images,password) VALUES ('$full','$user','$email','$phone','$role','$shop','$state','$cit', '$filename', '$pass')";
  ...
  <div>
     ...
     Fullname:  <input type="text" name="fullname" placeholder="Fullname" required><br><br>
      ...
      <select name="state">
          ...
          while($rows = mysqli_fetch_assoc($rest1)){

          ?>
          <option value="<?php echo $rows["name"]?>"> <?php echo $rows["name"]?></option>
          <?php } ?>
      </select><br><br>
      City: <input type="text" name="cit" placeholder="The city where you Reside"><br><br>
      Company/Shop Address: <input type="text" name="shop" placeholder="Company/Shop Address" required><br><br>
